...
import { Heart, MoreVertical, Play, Pause, Plus } from 'lucide-react'
    ...
    >
      ...
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className={`truncate font-semibold ${isCurrent ? 'text-primary' : 'text-foreground'}`}>
              {track.title}
            </span>
            {isPlaying && (
          ...
          <span className="block truncate text-[11.5px] text-muted-foreground">{track.artist}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <div className="relative">
          ...
          {showMenu && (
            <div
              className="absolute right-0 top-full z-50 mt-1 w-48 rounded-xl border border-border/80 bg-card/95 p-1.5 shadow-xl backdrop-blur-xl"
