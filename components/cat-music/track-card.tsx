    ...
    <div className="group relative flex flex-col gap-0.5 rounded-[10px] p-0 transition-colors hover:bg-white/[0.04]">
      <div className="relative aspect-square w-full overflow-hidden rounded-[10px] bg-white/[0.04]">
        ...
        <button
          ...
          className={`absolute right-1 bottom-1 flex size-6 items-center justify-center rounded-full bg-[#1DB954] text-white shadow-lg shadow-[#1DB954]/40 transition-all ${
        ...
        {isCurrent && isPlaying && (
          <div className="absolute inset-0 flex items-end justify-center gap-0.5 p-1 bg-black/25">
            <span className="w-0.5 bg-[#1DB954] rounded-full h-1.5 animate-pulse" />
            <span className="w-0.5 bg-[#1DB954] rounded-full h-2 animate-pulse delay-75" />
            <span className="w-0.5 bg-[#1DB954] rounded-full h-1 animate-pulse delay-150" />
          </div>
        )}
      ...
      <div className="min-w-0 px-0">
        <h4 className={`truncate text-[11px] font-semibold leading-tight ${isCurrent ? 'text-[#1DB954]' : 'text-white'}`}>
          {track.title}
        </h4>
