...
import { addConversationMembers, searchUsers, type UserSearchResult } from '@/app/actions/chat'
...
export function AddMembersModal({
  ...
  const handleAdd = async (userId: string) => {
    ...
    setAdding(userId)
    try {
      await addConversationMembers(conversationId, [userId])
  ...
  const dialogTitle = t.addMembersTitle
  const searchPlaceholder = t.addMembersSearchPlaceholder
  const searchLabel = t.addMembersSearchLabel
  const closeLabel = t.addMembersCloseLabel
  const noQueryText = t.addMembersNoQuery
  const noResultsPrefix = t.addMembersNoResults
  const addLabel = t.addMembersActionLabel
  const addedLabel = t.addMembersAddedLabel
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <div className="thin-scroll flex-1 overflow-y-auto p-2" role="listbox" aria-label={searchLabel}>
              {!query.trim() ? (
                ...
                results.map((u) => {
                  ...
                  return (
                    <button
                      ...
                      role="option"
                      aria-selected={false}
                      onClick={() => !isAdded && handleAdd(u.id)}
                    ...
                    >
                      ...
                      {adding === u.id ? (
                        ...
                        <span className="shrink-0 text-[11px] font-medium text-muted-foreground">{addLabel}</span>
                      )}
                    </button>
                  )
                })
