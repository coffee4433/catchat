...
export function ScreenPicker() {
  ...
  const handleSelect = async (sourceId: string) => {
    ...
    if (api) await api.select(sourceId)
    setOpen(false)
    setSources([])
  }
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-foreground">Choose what to share</h2>
              <button
            ...
            <div className="grid max-h-80 grid-cols-2 gap-3 overflow-auto">
              {sources.length === 0 && (
                <p className="col-span-2 py-8 text-center text-sm text-muted-foreground">Loading sources...</p>
              )}
              {sources.map((source) => (
                <button
                  key={source.id}
                ...
                >
                  {source.thumbnail ? (
                    <img src={source.thumbnail} alt={source.name} className="w-full rounded object-cover" style={{ aspectRatio: '16/10' }} />
                  ) : (
                    <div className="w-full rounded bg-muted" style={{ aspectRatio: '16/10' }} />
                  )}
                  <span className="w-full truncate text-center text-xs text-foreground">{source.name}</span>
                </button>
              ))}
            </div>
          </motion.div>
