...
function detectSteps(chunk) {
  ...
  if (lower.includes('packaging and publishing plugin') || lower.includes('plugin manifest created')) {
    events.push({ step: 'prepare', state: 'active' })
  }
  if (lower.includes('plugin manifest created') || lower.includes('plugin release saved')) {
    events.push({ step: 'prepare', state: 'done' })
    events.push({ step: 'build', state: 'active' })
  }
  if (lower.includes('building installer and packaging artifact')) {
    events.push({ step: 'prepare', state: 'done' })
    events.push({ step: 'build', state: 'active' })
  }
  if (lower.includes('uploading plugin release assets')) {
    events.push({ step: 'build', state: 'done' })
    events.push({ step: 'upload', state: 'active' })
  }
  if (lower.includes('github release created')) {
    events.push({ step: 'upload', state: 'done' })
    events.push({ step: 'publish', state: 'active' })
  }
  if (lower.includes('created git tag') || lower.includes('git commit')) {
    events.push({ step: 'publish', state: 'done' })
    events.push({ step: 'git', state: 'active' })
  }
  if (lower.includes('pushed plugin files') || lower.includes('pushed to git and published plugin')) {
  ...
  if ((lower.includes('building') || lower.includes('electron-builder') || lower.includes('packaging')) && !lower.includes('plugin')) {
    events.push({ step: 'prepare', state: 'done' })
    events.push({ step: 'build', state: 'active' })
  }
...
ipcMain.handle('delete:plugin', async (_event, pluginId) => {
  ...
  if (cfg.projectPath && pluginId) {
    const pluginDir = path.join(cfg.projectPath, 'plugins', pluginId)
